...
class DenseMatching:
    ...
    def Propagation(self, thld_zncc_propagation=0.5, thld_confidence=0.01, w=2, N_window=2, epsilon=1):
        ...
        for i in pbar:
            pbar.set_description(f'Working on image {self.i} and {self.j}, Propagating on Seeds. Current Length of Seeds = {len(self.Seeds)-1}')
    ...
    def plot(self):
        ...
        axs = axs.flatten()
        axs[0].imshow(self.Imgs[0])
        axs[1].imshow(self.Imgs[1])
        for row in self.Seeds:
            _, _, li = row
            a, b = li
            axs[0].plot([a[1]], [a[0]], 'o')
            axs[1].plot([b[1]], [b[0]], 'o')
        plt.savefig(f"fig{self.i}{self.j}")
