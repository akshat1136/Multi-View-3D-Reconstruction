...
class FundamentalCameraMatrix:
    ...
    def create_intrinsic_K(self):
        ...
        #                   [0, 0, 1]])
        #self.K = np.array([[1175, 0, self.x0],
        #                   [0, 1175, self.y0],
        #                   [0, 0, 1]])
    ...
    def plot_epipolar_epipole(self, F, points1, points2, epipole):
        '''
    ...
    def camera_matrix(self, reprojection):              
        ...
        return (num_points == num_points.max()).sum()
    ...
    def Calculate_F_E_C_Matrix(self, reprojection=False, loop_F_mat=200, thld_reprojection=30, thld_F_inliers=0.2, plot_epipolar_line=False, plot_3D_P2=False, plot_P2_best=False, plot_epipole=False):        
        ...
        reprojection_err = self.Imgs[0].shape[0] * self.Imgs[0].shape[1]
        pbar = tqdm(range(200), file=sys.stdout)
        ...
        for i in pbar:
            ...
            if check and self.reprojection_err < reprojection_err:
                data = self.F, self.E, self.K, self.P1, self.M1, self.K, self.P2, self.M2, self.reprojection_err, self.inliers_pnts
                reprojection_err = self.reprojection_err
            pbar.set_description(">>> Current Best Reprojection Error :" + str(reprojection_err)[:6] + " with Inliers :" + str(int(self.inliers_pnts.sum())) + '/' + str(self.points1.shape[0]))
        pbar.close()
        
        if plot_epipolar_line:
            eight_points = np.random.choice(range(self.points1[data[-1]].shape[0]), 5, replace=False)
            self.plot_epipolar_epipole(self.F, self.points1[data[-1]][eight_points], self.points2[data[-1]][eight_points], plot_epipole)
        ...
        self.F, self.E, self.K, self.P1, self.M1, self.K, self.P2, self.M2, self.reprojection_err, self.inliers_pnts = data
        return self.F, self.E, self.K, self.P1, self.M1, self.K, self.P2, self.M2, self.reprojection_err, self.inliers_pnts